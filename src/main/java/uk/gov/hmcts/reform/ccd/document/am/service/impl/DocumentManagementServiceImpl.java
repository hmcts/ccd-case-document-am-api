...
public class DocumentManagementServiceImpl implements DocumentManagementService {
    ...
    @Override
    public boolean patchDocumentMetadata(CaseDocumentMetadata body, String serviceAuthorization, String userId,
                                         String userRoles) {
        LinkedMultiValueMap<String, Object> bodyMap = new LinkedMultiValueMap<>();

        DocumentUpdate documentUpdate = new DocumentUpdate();
        documentUpdate.setDocumentId(UUID.fromString(body.getDocuments().get(0).getId()));

        Map<String, String> metadataMap = new HashMap<>();
        metadataMap.put("jurisdictionId", body.getJurisdictionId());
        metadataMap.put("caseId", body.getCaseId());
        metadataMap.put("caseTypeId", body.getCaseTypeId());

        documentUpdate.setMetadata(metadataMap);

        bodyMap.set("documents", (documentUpdate));
