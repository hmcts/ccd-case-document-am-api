...
#    And   the response has all the details as expected.
  ...
  @S-106
  Scenario: must get an error response when Ex-UI tries to access get hashtoken by document Id API
    Given a user with [an active caseworker profile in CCD with full permissions on a document field],
    And a successful call [by another privileged user to upload a document with mandatory metadata] as in [Default_Document_Upload_Data],
    When a request is prepared with appropriate values,
    And the request [contains the document Id from just uploaded document],
  ...
  @S-107
  Scenario: must get an error response when API-Gateway tries to access get hashtoken by document Id API
    Given a user with [an active caseworker profile in CCD with full permissions on a document field],
    And a successful call [by another privileged user to upload a document with mandatory metadata] as in [Default_Document_Upload_Data],
    When a request is prepared with appropriate values,
    And the request [contains the document Id from just uploaded document],
  ...
  @S-108
  Scenario: must get an error response when CCD Data Store tries to access get hashtoken by document Id API
    Given a user with [an active caseworker profile in CCD with full permissions on a document field],
    And a successful call [by another privileged user to upload a document with mandatory metadata] as in [Default_Document_Upload_Data],
    When a request is prepared with appropriate values,
    And the request [contains the document Id from just uploaded document],
