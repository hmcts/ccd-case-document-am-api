...
Feature: F-001: Get Document Metadata by Document ID
  ...
  @S-001 @Retryable(statusCodes={400,409,502})
  Scenario: must successfully get document metadata by document ID
    ...
    And   it is submitted to call the [Get Document Metadata by Document ID] operation of [CCD Case Document AM API] [after] delaying [30] seconds,
