...
Feature: F-001: Retrieval of metadata from doc-store for given document id
  ...
  @S-003
  Scenario: Successful retrieval of metadata from doc-store for given document id with optional fields linked with user token
    Given a user with [an active profile in CCD]
    And   a user with [an active caseworker profile in CCD with full permission on specific document field]
    And   a successful call [by a privileged user to upload a document with mandatory metadata] as in [Default_Document_Upload_Data]
    When  a request is prepared with appropriate values
    And   the request[contains valid data for optional fields i.e. userId and userRoles linked with user token]
    And   it is submitted to call the [retrieval of metadata from doc-store for given document id] operation of [CCD data store]
    Then  a positive response is received
  ...
  @S-004
  Scenario: Successful retrieval of metadata from doc-store for given document id with optional fields not linked with user token
    Given a user with [an active profile in CCD]
    And   a user with [an active caseworker profile in CCD with full permission on specific document field]
    And   a successful call [by a privileged user to upload a document with mandatory metadata] as in [Default_Document_Upload_Data]
    When  a request is prepared with appropriate values
    And   the request[contains valid data for optional fields i.e. userId and userRoles not linked with user token]
    And   it is submitted to call the [retrieval of metadata from doc-store for given document id] operation of [CCD data store]
    Then  a positive response is received
    And   the response [contains the case detail for the updated case, along with an HTTP 200 OK]
    And   the response has all other details as expected
  ...
  @S-005
  Scenario: Successful retrieval of metadata from doc-store for given document id with optional fields non-existing userId
    Given a user with [an active profile in CCD]
    And   a user with [an active caseworker profile in CCD with full permission on specific document field]
    And   a successful call [by a privileged user to upload a document with mandatory metadata] as in [Default_Document_Upload_Data]
    When  a request is prepared with appropriate values
    And   the request[contains valid data for optional fields i.e. userId and userRoles non-existing userId]
    And   it is submitted to call the [retrieval of metadata from doc-store for given document id] operation of [CCD data store]
    Then  a positive response is received
    And   the response [contains the case detail for the updated case, along with an HTTP 200 OK]
    And   the response has all other details as expected
  ...
  @S-006
  Scenario: Successful retrieval of metadata from doc-store for given document id with optional fields non-existing userRoles
    Given a user with [an active profile in CCD]
    And   a user with [an active caseworker profile in CCD with full permission on specific document field]
    And   a successful call [by a privileged user to upload a document with mandatory metadata] as in [Default_Document_Upload_Data]
    When  a request is prepared with appropriate values
    And   the request[contains valid data for optional fields i.e. userId and userRoles non-existing userRoles]
    And   it is submitted to call the [retrieval of metadata from doc-store for given document id] operation of [CCD data store]
    Then  a positive response is received
    And   the response [contains the case detail for the updated case, along with an HTTP 200 OK]
    And   the response has all other details as expected
